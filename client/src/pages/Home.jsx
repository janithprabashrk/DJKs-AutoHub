...
export default function Home() {
  ...
  const slides = [
    ...
    },
    {
      ...
      description: 'Sophistication meets performance in every journey'
    },
    {
      backgroundImage: '../../car2.jpg',
      title: 'Pure Performance',
      subtitle: 'Sport Cars',
      textColor: 'from-red-500 to-amber-400',
      description: 'Unleash the power of speed and precision'
    },
    {
      backgroundImage: 'https://images.unsplash.com/photo-1570125909232-eb263c188f7e?q=80&w=2071',
      title: 'Group Travel',
      subtitle: 'Luxury Buses',
      textColor: 'from-teal-400 to-cyan-500',
      description: 'Comfortable and spacious solutions for group journeys'
    }
  ];
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      setActiveIndex((prevIndex) => (prevIndex + 1) % slides.length);
      setTimeout(() => setIsTransitioning(false), 1500);
    }, 7000); // Increased to 7 seconds to allow for transition animation
  ...
  return (
    <>
      <div className="overflow-x-hidden">
        ...
        <div 
          ref={heroSectionRef}
          className="relative min-h-[115vh] overflow-hidden -mt-24 bg-gradient-to-r from-blue-900 via-black to-orange-900"
        >
            {/* Permanent particle overlay - removed mouse interaction */}
            <div className="absolute inset-0 z-10 pointer-events-none overflow-hidden">
              {[...Array(150)].map((_, i) => {
                const randomSize = Math.random() * 6 + 2;
                const randomX = Math.random() * 100;
                const randomY = Math.random() * 100;
                const uniqueDelay = Math.random() * 5;
                const uniqueDuration = 7 + Math.random() * 8;
                ...
                return (
                  <div 
                    ...
                    style={{
                      ...
                      background: `rgba(${56 + Math.random() * 100}, ${182 + Math.random() * 73}, ${255}, ${0.3 + Math.random() * 0.4})`,
                      boxShadow: `0 0 ${10 + Math.random() * 15}px rgba(56, 182, 255, 0.6)`,
                      willChange: 'transform, opacity',
                      animationDelay: `${uniqueDelay}s`,
                      animationDuration: `${uniqueDuration}s`
                    }}
                  />
                );
              })}
            ...
            {/* Particle transition overlay - keep this for transitions */}
            {isTransitioning && (
              <div 
                className={`absolute inset-0 z-30 pointer-events-none overflow-hidden ${
                  transitionDirection === 'next' ? 'particle-transition-next' : 'particle-transition-prev'
                }`}
              >
                {[...Array(120)].map((_, i) => (
                  <div 
                    ...
                    style={{
                      ...
                      opacity: Math.random(),
                      background: `rgba(${56 + Math.random() * 100}, ${182 + Math.random() * 73}, ${255}, ${0.3 + Math.random() * 0.7})`,
                      boxShadow: `0 0 ${10 + Math.random() * 20}px rgba(56, 182, 255, 0.8)`,
            ...
            {slides.map((slide, index) => (
              ...
              >
                ...
                ></div>
                <div className="flex flex-col gap-10 pt-52 md:pt-48 pb-36 px-3 max-w-6xl mx-auto justify-center h-full">
                  <motion.div
                    initial="hidden"
                    animate="visible"
                    variants={fadeInUp}
                    className="max-w-3xl"
                  >
                    {/* Decorative tech bracket */}
                    <div className="absolute -left-5 h-24 w-1 bg-gradient-to-b from-cyan-400 to-blue-500 opacity-70 rounded-full blur-[2px]"></div>
                    ...
                    >
                      {slide.title.split(' ').map((word, i) => (
                        <React.Fragment key={i}>
                          {i > 0 && <br className="mb-4" />}
                          <span className="inline-block hover:scale-105 transition-transform hover:text-shadow-glow mb-3">
                            {word}
                          </span>
                        </React.Fragment>
                      ))}
                    ...
                    <div
                      className={`text-gray-200 text-lg sm:text-xl transform transition-all duration-1000 mt-8 ${
                    ...
                    >
                      ...
                      after:content-['_•'] after:text-cyan-400">
                        {slide.subtitle}
                      </span>
                      <br/>
                      <span className="bold text-gray-300 mt-4 block">{slide.description}</span>
                    </div>
                    
                    <div className="mt-10 space-y-4">
                      <a
                        href="/search"
                        className={`group flex items-center gap-3 w-fit text-sm sm:text-base text-cyan-400 font-bold hover:text-cyan-300 transform transition-all duration-1000 
                        border border-cyan-400/30 hover:border-cyan-400/70 px-5 py-2 rounded-md backdrop-blur-sm bg-black/10 hover:bg-black/20 ${
                      ...
                      <a
                        href="/contact-us"
                        className={`group flex items-center gap-3 w-fit text-sm sm:text-base text-white/70 hover:text-white transition-all duration-1000 
                        px-5 py-2 rounded-md ${
                          index === activeIndex ? 'translate-x-0 opacity-100 delay-900' : '-translate-x-20 opacity-0'
                        }`}
                      >
                        <span>Contact Us</span>
                        <span className="group-hover:translate-x-3 transition-transform duration-300 opacity-0 group-hover:opacity-100">→</span>
                      </a>
                    </div>
                    
                    {/* Tech decorative element */}
                    <div className="absolute -bottom-10 left-0 h-1 w-40 bg-gradient-to-r from-cyan-400 to-transparent rounded-full"></div>
                  </motion.div>
                </div>
              </div>
            ))}
            ...
            {/* Navigation Buttons */}
                ...
                {[0,1,2,3,4,5,6].map((index) => (
                  ...
                  >
                  <span className={`block h-1 w-10 backdrop-blur-sm transition-all duration-500 ${
                    index === activeIndex 
                    ? 'bg-gradient-to-r from-blue-500 to-cyan-400 shadow-lg shadow-cyan-500/50 '
                    : 'bg-white/30'
                  }`}
                  style={{
                    transform: index === activeIndex ? 'scaleX(1.2) scaleY(2.5)' : 'scaleX(1) scaleY(1)',
                    borderRadius: '4px' // Increased border radius for a more rounded look
                  }}
                  />
                  <span className={`absolute -bottom-1 left-0 h-3 w-3 transition-all duration-300 ${
                    index === activeIndex 
                  ...
                  style={{
                    ...
                    transform: index === activeIndex ? 'translateX(4px)' : 'translateX(0)',
                    filter: 'blur(1px)',
                    borderRadius: '50%', // Added explicit border radius for the glow effect
                    animation: index === activeIndex ? 'pulse 2s infinite' : 'none'
                  }}
                  />
                  <style jsx>{`
                    @keyframes pulse {
                ...
                <style jsx>{`
                  .particle-transition-next, .particle-transition-prev {
                    animation: fadeInOut 1.5s forwards;
                  }
                  
                  .particle-transition-next .particle {
                    animation: slideRightToLeft 1.5s cubic-bezier(0.23, 1, 0.32, 1) forwards;
                    border-radius: 50%; /* Added border-radius to particles */
                  }
                  
                  .particle-transition-prev .particle {
                    animation: slideLeftToRight 1.5s cubic-bezier(0.23, 1, 0.32, 1) forwards;
                    border-radius: 50%; /* Added border-radius to particles */
                  }
                  ...
                  @keyframes slideRightToLeft {
                    0% { 
                      transform: translateX(100%) scale(0.2) rotate(0deg); 
                      filter: blur(5px);
                    }
                    100% { 
                      transform: translateX(-300%) scale(1.5) rotate(360deg); 
                      filter: blur(0);
                    }
                  }
                  ...
                  .particle {
                    ...
                    border-radius: 50%; /* Reinforced border-radius for all particles */
                  ...
                  @keyframes float {
                    ...
                    40% {
                      transform: translateY(-5px) translateX(-8px) rotate(-25deg);
                    }
                    60% {
                      transform: translateY(8px) translateX(9px) rotate(15deg);
                    }
                    80% {
                      transform: translateY(-3px) translateX(-5px) rotate(-10deg);
                    }
                    100% {
                      transform: translateY(6px) translateX(2px) rotate(20deg);
                    }
                  }
                `}</style>
                </div>
                ...
                >
                  <div className="max-w-7xl mx-auto px-4">
                    <motion.h2 
                      ...
                      className="text-4xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-blue-500 mb-8 text-center"
                    >
                      Featured Vehicles          
                    </motion.h2>
                ...
                {/* Listing Results Section with Animations */}
                <motion.div
                ...
                {/* About Us Footer with Animations */}
                <motion.footer 
